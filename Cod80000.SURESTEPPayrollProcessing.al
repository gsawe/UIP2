Codeunit 80000 "AU Payroll Processing"
{
    ...
    var
        ...
        CoopParameters: Option "none",shares,loan,"loan Interest","Emergency loan","Emergency loan Interest","School Fees loan","School Fees loan Interest",Welfare,Pension,NSSF,Overtime,DevShare,NHIF,Holiday;
        PayrollType: Code[20];
    ...
    procedure FFfnProcesspayroll(strEmpCode: Code[20]; dtDOE: Date; curBasicPay: Decimal; blnPaysPaye: Boolean; blnPaysNssf: Boolean; blnPaysNhif: Boolean; SelectedPeriod: Date; dtOpenPeriod: Date; Membership: Text[30]; ReferenceNo: Text[30]; dtTermination: Date; blnGetsPAYERelief: Boolean; Dept: Code[20]; PayrollCode: Code[20])
    var
    ...
    begin
        ...
        if curBasicPay > 0 then begin
            ...
            curDefinedContrib := curNSSF + EmployeeP."Insurance Premium";//curNSSF; //(curNSSF + curPensionStaff + curNonTaxable) - curMorgageReliefAmount
           ...
           Coopparameters::none);
            ...
            //Message('CurTaxable%1CurSal%2Defined%3Pension%4Cuoo1%5curHosp%6CurNon%7Gross%8',curTaxablePay,curSalaryArrears,curDefinedContrib,curMaxPensionContrib,curOOI,curHOSP,curNonTaxable,curGrossTaxable);
            //Taxable Benefit
            txBenefitAmt := 0;
            if EmpSalary.Get(strEmpCode) then begin
                PensionNe := 0;
                PensionNe := 0.05 * EmpSalary."Basic Pay";
                if EmpSalary."Pays NSSF" = false then begin
                    if fnCheckPaysPension(strEmpCode, SelectedPeriod) = true then begin
                        if (EmpSalary."Basic Pay" * 0.5) > 20000 then begin
            ...
            //Message('TaxBenefit%1',PensionNe);
            ...
            curTaxablePay := curTaxablePay - PensionNe; // + txBenefitAmt;

            curTaxCharged := ROUND(fnGetEmployeePaye(curTaxablePay), 1, '>') - (curInsuranceReliefAmount + CurrInsuranceRel);

            if curTaxCharged = 0 then
                curTaxCharged := curTaxCharged
            else
                curTaxCharged := (curTaxCharged) - curReliefPersonal;
            ...
            TGroupOrder := 6;
            TSubGroupOrder := 7;
            //Message('Taxable%1',curTransAmount);
            ...
            fnUpdatePeriodTrans(strEmpCode, 'PAYE', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
             curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, salariesAcc, Journalpostas::Credit,
            ...
            if blnPaysNhif then begin
                ...
                TGroupOrder := 6;
                TSubGroupOrder := 9;
                fnUpdatePeriodTrans(strEmpCode, 'NHIF Re', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
                 curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', Journalpostas::" ", Journalpostingtype::" ", '',
                 Coopparameters::none);
            end;
    ...
    //procedure ProcessEmployeeNSSF(EmployeeNo:Code[40],PayrollPeriod:[40],pr)
    ...
    begin
        ...
        //     Report.Run(51512591, false, false, ObjPRTransactioons);
        //Report.RunModal(51512591, false);//TO RELOOK
        // Commit();
    end;
