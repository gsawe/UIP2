...
Table 51512056 "Funds Transfer Header"
{
    ...
    fields
    {
        ...
        {
            CalcFormula = sum("Funds Transfer Lines"."Amount to Receive" where("Document No" = field("No.")));
            Editable = false;
            FieldClass = FlowField;
        }
        field(23; "Total Line Amount(LCY)"; Decimal)
        {
            CalcFormula = sum("Funds Transfer Lines"."Amount to Receive (LCY)" where("Document No" = field("No.")));
            Editable = false;
            FieldClass = FlowField;
        }
